...
// 监听会话列表变化，确保新创建的会话能立即显示
watch(() => sessionStore.sessionList, () => {
  // 强制触发组件重新渲染
}, { deep: true });
...
// 切换会话
async function handleChange(item: ConversationItem<ChatSessionVo>) {
  sessionStore.setCurrentSession(item);

  // 先加载聊天记录
  await chatStore.requestChatList(item.id!);
