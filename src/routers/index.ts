...
router.beforeEach(
  async (
    ...
    // 5、判断是否有 Token，没有则打开登录弹框
    if (!userStore.token) {
      // 如果是聊天相关页面，打开登录弹框而不是直接跳转
      if (to.path.startsWith('/chat')) {
        userStore.openLoginDialog();
        // 如果是聊天页面但没有token，重定向到未登录状态
        if (to.path === '/chat' || to.path.startsWith('/chat/') && to.params.id !== 'not_login') {
          return next('/chat/not_login');
        }
      } else {
        // 非聊天页面，使用原有的登出逻辑
        userStore.logout();
