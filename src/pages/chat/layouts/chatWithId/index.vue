...
async function startSSE(chatContent: string) {
  ...
  try {
    // 检查当前会话是否是临时会话（以chat_开头），如果是且是第一条消息，则更新会话标题
    const currentSessionId = route.params?.id as string;
    if (currentSessionId && currentSessionId.startsWith('chat_') && bubbleItems.value.length === 0) {
      // 这是临时会话的第一条消息，需要更新会话标题为用户的问题内容
      await sessionStore.updateSession({
        id: currentSessionId,
        sessionTitle: chatContent.slice(0, 20) || '新对话', // 取前20个字符作为标题
          ...
          <div class="sender-header p-12px pt-6px pb-0px">
            <Attachments :items="filesStore.filesList" :hide-upload="true" @delete-card="handleDeleteCard">
              <template #prev-button="{ show, onScrollLeft }">
                <div
                  v-if="show"
                  class="prev-next-btn left-8px flex-center w-22px h-22px rounded-8px border-1px border-solid border-[rgba(0,0,0,0.05)] c-[rgba(0,0,0,.4)] hover:bg-#f3f4f6 bg-#fff font-size-10px"
                  @click="onScrollLeft"
                >
                  <el-icon>
                    <ArrowLeftBold />
              ...
              <template #next-button="{ show, onScrollRight }">
                <div
                  v-if="show"
                  class="prev-next-btn right-8px flex-center w-22px h-22px rounded-8px border-1px border-solid border-[rgba(0,0,0,0.05)] c-[rgba(0,0,0,.4)] hover:bg-#f3f4f6 bg-#fff font-size-10px"
                  @click="onScrollRight"
                >
                  <el-icon>
                    <ArrowRightBold />
...
/* 深色模式特殊适配 */
:global(.dark-mode) .chat-with-id-container {
  /* 深色模式下的特殊样式调整 */
  background-color: var(--el-bg-color);
  color: var(--el-text-color-primary);
